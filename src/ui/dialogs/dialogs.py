...
def show_voice_mode_settings_dialog(parent: tk.Tk, current_config: dict, save_callback: callable) -> None:
    """Show dialog to configure voice mode settings including LLM selection.
    
    Args:
        parent: Parent window
        current_config: Current voice mode configuration
        save_callback: Function to call when saving settings
    """
    from settings.settings import _DEFAULT_SETTINGS
    default_config = _DEFAULT_SETTINGS.get("voice_mode", {})
    
    # Create dialog
    dialog = create_toplevel_dialog(parent, "Voice Mode Settings", "800x600")
    ...
    # Add description
    desc_label = ttk.Label(model_frame, text="Select the language model to use for voice mode conversations:",
                          wraplength=700)
    desc_label.grid(row=0, column=0, columnspan=2, sticky="w", padx=10, pady=(10, 20))
    
    # Create model selection UI
    model_vars = _create_models_tab(
        model_frame,
        current_config.get("openai_model", default_config.get("openai_model", "gpt-4")),
        current_config.get("perplexity_model", default_config.get("perplexity_model", "sonar-reasoning-pro")),
        current_config.get("grok_model", default_config.get("grok_model", "grok-1")),
        current_config.get("ollama_model", default_config.get("ollama_model", "llama3")),
        current_config.get("anthropic_model", default_config.get("anthropic_model", "claude-3-sonnet-20240229"))
    )
    
    # AI Provider selection
    ttk.Label(model_frame, text="AI Provider:").grid(row=5, column=0, sticky="w", padx=10, pady=(20, 10))
    provider_var = tk.StringVar(value=current_config.get("ai_provider", default_config.get("ai_provider", "openai")))
    provider_combo = ttk.Combobox(model_frame, textvariable=provider_var, 
                                  values=["openai", "perplexity", "grok", "ollama", "anthropic"],
                                  state="readonly", width=30)
    provider_combo.grid(row=5, column=1, sticky="w", padx=10, pady=(20, 10))
    
    # Create Settings tab
    settings_frame = ttk.Frame(notebook)
    notebook.add(settings_frame, text="Voice Settings")
    settings_frame.columnconfigure(1, weight=1)
    
    # System prompt
    ttk.Label(settings_frame, text="System Prompt:").grid(row=0, column=0, sticky="nw", padx=10, pady=(10, 5))
    system_prompt_text = tk.Text(settings_frame, height=6, width=60, wrap=tk.WORD)
    system_prompt_text.grid(row=0, column=1, sticky="ew", padx=10, pady=(10, 5))
    system_prompt_text.insert("1.0", current_config.get("system_prompt", default_config.get("system_prompt", "")))
    
    # Temperature
    ttk.Label(settings_frame, text="AI Temperature:").grid(row=1, column=0, sticky="w", padx=10, pady=(10, 5))
    temp_frame = ttk.Frame(settings_frame)
    temp_frame.grid(row=1, column=1, sticky="ew", padx=10, pady=(10, 5))
    
    temp_var = tk.DoubleVar(value=current_config.get("ai_temperature", default_config.get("ai_temperature", 0.7)))
    temp_scale = tk.Scale(temp_frame, from_=0.0, to=2.0, resolution=0.1,
                         orient=tk.HORIZONTAL, variable=temp_var, length=300)
    temp_scale.pack(side=tk.LEFT, padx=(0, 10))
    
    temp_label = ttk.Label(temp_frame, text=f"{temp_var.get():.1f}")
    temp_label.pack(side=tk.LEFT)
    temp_scale.config(command=lambda v: temp_label.config(text=f"{float(v):.1f}"))
    
    # TTS Provider
    ttk.Label(settings_frame, text="TTS Provider:").grid(row=2, column=0, sticky="w", padx=10, pady=(10, 5))
    tts_var = tk.StringVar(value=current_config.get("tts_provider", default_config.get("tts_provider", "openai")))
    tts_combo = ttk.Combobox(settings_frame, textvariable=tts_var,
                            values=["openai", "elevenlabs"],
                            state="readonly", width=30)
    tts_combo.grid(row=2, column=1, sticky="w", padx=10, pady=(10, 5))
    
    # TTS Voice
    ttk.Label(settings_frame, text="TTS Voice:").grid(row=3, column=0, sticky="w", padx=10, pady=5)
    voice_var = tk.StringVar(value=current_config.get("tts_voice", default_config.get("tts_voice", "nova")))
    voice_combo = ttk.Combobox(settings_frame, textvariable=voice_var, width=30, state="readonly")
    voice_combo.grid(row=3, column=1, sticky="w", padx=10, pady=5)
    
    # Store voice mappings (id -> name)
    voice_mappings = {}
    
    def update_voice_combo(*args):
        """Update voice combo when TTS provider changes."""
        provider = tts_var.get()
        
        # Show progress dialog
        progress_dialog = create_toplevel_dialog(dialog, "Fetching Voices", "300x100")
        progress_frame = ttk.Frame(progress_dialog, padding=20)
        progress_frame.pack(fill=tk.BOTH, expand=True)
        ttk.Label(progress_frame, text=f"Fetching {provider} voices...").pack(pady=(0, 10))
        progress = ttk.Progressbar(progress_frame, mode="indeterminate")
        progress.pack(fill=tk.X)
        progress.start()
        progress_dialog.update()
        ...
        try:
            # Fetch voices
            voices = _fetch_tts_voices(provider)
            
            if voices:
                # Clear previous mappings
                voice_mappings.clear()
                
                # Build voice list and mappings
                voice_names = []
                ...
                for voice in voices:
                    ...
                    # Create display name
                    if voice_desc:
                        display_name = f"{voice_name} - {voice_desc}"
                    else:
                        display_name = voice_name
                    
                    voice_names.append(display_name)
                    voice_mappings[display_name] = voice_id
                    
                    # Check if current voice matches
                ...
                # If current voice not found, select first one
                if not found_current and voice_names:
                    voice_var.set(voice_names[0])
                    
            else:
                # No voices or error - check if API key is missing
                if provider == "openai" and not os.getenv("OPENAI_API_KEY"):
                    messagebox.showwarning("API Key Missing", 
                        "OpenAI API key not found. Please update your API keys in Settings menu.")
                elif provider == "elevenlabs" and not os.getenv("ELEVENLABS_API_KEY"):
                    messagebox.showwarning("API Key Missing", 
                        "ElevenLabs API key not found. Please update your API keys in Settings menu.")
                else:
                    messagebox.showwarning("Voice Fetch Failed", 
                        f"Could not fetch voices for {provider}. Using default voice list.")
                
                # Use fallback voice lists
                if provider == "openai":
                    voice_combo['values'] = ["nova", "alloy", "echo", "fable", "onyx", "shimmer"]
                    if voice_var.get() not in voice_combo['values']:
                        voice_var.set("nova")
                else:
                    voice_combo['values'] = []
                    
        except Exception as e:
            logging.error(f"Error updating voice combo: {e}")
            messagebox.showerror("Error", f"Failed to fetch voices: {str(e)}")
            
        finally:
            progress_dialog.destroy()
    
    # Bind TTS provider change to update voices
    tts_var.trace('w', update_voice_combo)
    
    # Initial voice population
    dialog.after(100, update_voice_combo)
    
    # Enable interruptions
    interrupt_var = tk.BooleanVar(value=current_config.get("enable_interruptions", 
                                                           default_config.get("enable_interruptions", True)))
    interrupt_check = ttk.Checkbutton(settings_frame, text="Enable voice interruptions",
                                     variable=interrupt_var)
    interrupt_check.grid(row=4, column=0, columnspan=2, sticky="w", padx=10, pady=(10, 5))
    ...
    def save_settings():
        """Save the voice mode settings."""
        # Get the appropriate model based on selected provider
        provider = provider_var.get()
        if provider == "openai":
            model = model_vars['openai'].get()
        elif provider == "perplexity":
            model = model_vars['perplexity'].get()
        elif provider == "grok":
            model = model_vars['grok'].get()
        elif provider == "ollama":
            model = model_vars['ollama'].get()
        elif provider == "anthropic":
            model = model_vars['anthropic'].get()
        else:
            model = model_vars['openai'].get()
        
        # Get the actual voice ID from display name
        voice_display = voice_var.get()
        voice_id = voice_mappings.get(voice_display, voice_display)  # Fallback to display value if not in mappings
        
        # Call save callback with all settings
        save_callback(
            provider,
            model,
            temp_var.get(),
            system_prompt_text.get("1.0", tk.END).strip(),
            tts_var.get(),
            voice_id,  # Use the actual voice ID
            interrupt_var.get(),
            model_vars['openai'].get(),
            model_vars['perplexity'].get(),
            model_vars['grok'].get(),
            model_vars['ollama'].get(),
            model_vars['anthropic'].get()
        )
        dialog.destroy()
    
    def reset_settings():
        """Reset to default settings."""
        provider_var.set(default_config.get("ai_provider", "openai"))
        temp_var.set(default_config.get("ai_temperature", 0.7))
        system_prompt_text.delete("1.0", tk.END)
        system_prompt_text.insert("1.0", default_config.get("system_prompt", ""))
        tts_var.set(default_config.get("tts_provider", "openai"))
        # Voice will be set by update_voice_combo when TTS provider changes
        interrupt_var.set(default_config.get("enable_interruptions", True))
        
        # Reset model selections
        model_vars['openai'].set(default_config.get("openai_model", "gpt-4"))
        model_vars['perplexity'].set(default_config.get("perplexity_model", "sonar-reasoning-pro"))
        model_vars['grok'].set(default_config.get("grok_model", "grok-1"))
        model_vars['ollama'].set(default_config.get("ollama_model", "llama3"))
        model_vars['anthropic'].set(default_config.get("anthropic_model", "claude-3-sonnet-20240229"))
    
    ttk.Button(btn_frame, text="Reset", command=reset_settings).pack(side=tk.LEFT, padx=5)
